 ...
 *    - Fixed the return value for Control ARM_MCI_BUS_SPEED function
 *  Version 1.5
...
static int32_t Initialize (ARM_MCI_SignalEvent_t cb_event) {
  ...
  PIN_Configure (RTE_SD_CMD_PORT,  RTE_SD_CMD_PIN,  2U | MCI_SD_CMD_MODE);
  PIN_Configure (RTE_SD_CLK_PORT,  RTE_SD_CLK_PIN,  2U | MCI_SD_CLK_MODE);
  PIN_Configure (RTE_SD_DAT0_PORT, RTE_SD_DAT0_PIN, 2U | MCI_SD_DAT0_MODE);

  #if (MCI_BUS_WIDTH_4 != 0U)
  PIN_Configure (RTE_SD_DAT1_PORT, RTE_SD_DAT1_PIN, 2U | MCI_SD_DAT1_MODE);
  PIN_Configure (RTE_SD_DAT2_PORT, RTE_SD_DAT2_PIN, 2U | MCI_SD_DAT2_MODE);
  PIN_Configure (RTE_SD_DAT3_PORT, RTE_SD_DAT3_PIN, 2U | MCI_SD_DAT3_MODE);
  #endif

  /* Configure CD (Card Detect) Pin */
  #if (MCI_CD_PIN != 0U)
  GPIO_SetDir (RTE_SD_CD_PORT, RTE_SD_CD_PIN, GPIO_DIR_INPUT);
  PIN_Configure (RTE_SD_CD_PORT, RTE_SD_CD_PIN, MCI_SD_CD_MODE);
  #endif

  /* Configure WP (Write Protect) Pin */
  #if (MCI_WP_PIN != 0U)
  GPIO_SetDir (RTE_SD_WP_PORT, RTE_SD_WP_PIN, GPIO_DIR_INPUT);
  PIN_Configure (RTE_SD_WP_PORT, RTE_SD_WP_PIN, MCI_SD_WP_MODE);
  #endif

  /* Configure SD_PWR (Power Supply Enable) Pin */
  #if (MCI_PWR_PIN != 0U)
  PIN_Configure (RTE_SD_PWR_PORT, RTE_SD_PWR_PIN, 2U | MCI_SD_PWR_MODE);
